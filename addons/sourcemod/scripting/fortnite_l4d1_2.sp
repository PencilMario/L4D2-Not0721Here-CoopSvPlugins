...
public void OnRoundLiveCountdownPre(){
    for (int i = 1; i< MaxClients + 1; i++){
        if (IsClientInGame(i)){
            ...
            int commandflags = GetCommandFlags("sm_return");
            SetCommandFlags("sm_return", commandflags & ~FCVAR_CHEAT);
            FakeClientCommand(i, "sm_return");
            SetCommandFlags("sm_return", commandflags);
