...
public void OnPluginStart()
{
	...
	g_hCvarWay = CreateConVar("l4d2_lookatweapon_way", "8", "Bitwise: 1=double SHIFT, 2=chat command, 4=double F, 8=double R when full ammo");
...
void DisplayHint(int client, bool type)
{
	switch ( g_iCvarHints )
	{
		...
		{
			if(type)	PrintToChat(client, "[武器检视] 开启");
			if(!type)	PrintToChat(client, "[武器检视] 关闭");
		}
