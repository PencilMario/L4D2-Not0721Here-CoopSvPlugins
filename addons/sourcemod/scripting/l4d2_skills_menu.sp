...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_buy", sm_skills);
...
void ShowClientSkillMenu( int client )
{
	...
	menu.SetTitle("Skills: 主菜单");
	
	Format(display, sizeof display, "积分: %.1f | 倍率: %.2f", Skills_GetClientMoney(client), Skills_GetClientMoneyMultiplier(client));
	menu.AddItem("", display, ITEMDRAW_DISABLED);
	
	Format(display, sizeof display, "团队积分: %.1f | 倍率: %.2f", Skills_GetTeamMoney(), Skills_GetTeamMoneyMultiplier());
	menu.AddItem("", display, ITEMDRAW_DISABLED);
...
void ShowClientSkills( int client, int selection = 0 )
{
	...
	Format(name, sizeof name, "已有技能计数: %i", Skills_GetClientSkillsCount(client));
...
public int VSkillsHandler( Menu menu, MenuAction action, int client, int id )
{
	switch( action )
	{
		...
		case MenuAction_Select:
		{
			...
			Skills_PrintToChatAll("\x04%N \x05购买了 \x03%s", client, skill);
			sm_skills(client, 0);
		}
	}
...
public int VMenuUpgradeHandler( Menu menu, MenuAction action, int client, int index )
{
	switch( action )
	{
		...
		case MenuAction_Select:
		{
			...
			Skills_PrintToChat(client, "\x05你 \x04升级了 \x03%s", name);
			ShowClientSkills(client, index);
		}
	}
...
bool AllowedToOpenMenu(int cl, char[] reason, int maxlength)
{
	...
	{
		strcopy(reason, maxlength, "你需要在安全屋才能使用技能菜单.");
		return false;
	}

	strcopy(reason, maxlength, "已超出购买时间，请等待下一回合");
	return !g_isBlocked; 
}
