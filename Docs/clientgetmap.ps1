# 这是一个脚本，用aria2从http://sp2.0721play.icu/d/L4D2相关/MOD/战役图/ 下载文件
...
try {
    aria2c.exe
    Write-Output "Aria2c 可用"
}
catch {
    
    Write-Output "!!!你必须先重启一遍PC才能继续运行该脚本!!!"
    Write-Output "!!!按下回车键退出!!!"
    Read-Host
    throw "!!!你必须先重启一遍PC才能继续运行该脚本!!!"
}
...
$file_list=('B计划.rar', 'CEDA狂热.rar', 'F18之路.rar', 'ZMB13.rar', 'ZPTZ.rar',
    ...
    '广州增城v7.3.7z', '开路.rar', '异度神召(解谜&附解谜流程).rar', '恐怖之旅.rar', '惩罚者(阴间图).rar', '感染之城(长图).rar' ,
...
foreach ($file in $file_list) {
    if (-not (Test-Path "fin_$file")) { # 如果文件不存在
    ...
    New-Item "fin_$file" -ItemType File
