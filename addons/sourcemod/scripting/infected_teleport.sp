...
public void OnPluginStart()
{
	...
	log = new Logger("infected_teleport");

	// offset
	offset = intAbs(g_hMaxNavDistance.IntValue - g_hMaxDistance.IntValue);
	// cvar change hook
	g_hEnableLog.AddChangeHook(logchangedHandler);
...
public void logchangedHandler(ConVar convar, const char[] oldValue, const char[] newValue)
{
	if (g_hEnableLog.IntValue == 0) log.IgnoreLevel = LogType_Size;
	else log.IgnoreLevel = LogType_Info;
