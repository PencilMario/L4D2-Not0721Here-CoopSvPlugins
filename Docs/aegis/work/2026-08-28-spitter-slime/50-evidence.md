...
- Compile：`E:\GithubKu\L4d2_0721sv_plugins\spcomp.exe` 使用项目 SourcePawn `1.12.0.7221` 编译 `l4d2_spitter_slime.sp`，生成 `addons/sourcemod/plugins/optional/l4d2_spitter_slime.smx`，退出码为 `0`。最终编译器输出包含 `Code size: 40664 bytes`、`Data size: 257920 bytes` 和 `Total requirements: 315496 bytes`，未报告 warning；目标二进制大小为 `21731` bytes。
...
- 新插件使用 `L4D2_SpitterPrj` 创建原生可见 `spitter_projectile`，按每只喷吐者维护粘液引用、生成计时器和煤气罐引用。
- 粘液目标经过 `TR_TraceRayFilterEx` 可视性筛选，粘液命中和煤气罐范围伤害均使用 `SDKHooks_TakeDamage`。
- `L4D2_ActivateAbility_Spitter` 是 `IN_ATTACK` 替换入口；煤气罐使用可见模型，碰撞只触发一次爆炸，包含两组粒子和爆炸声音，并以直接速度写入击退。
- 换图可重建全局更新计时器；`l4d2_spitter_slime_enable` 热修改会停止/清理或重启当前喷吐者状态；煤气罐物理伤害缩放为 `0.0`；冷却设置失败会删除煤气罐并返回原版能力回退。
...
当前环境没有可用的 L4D2 实服，因此以下项目仍需部署后验证：粘液和煤气罐在引擎中的实际碰撞与显示、每只喷吐者上限和独立清理、随机可视目标命中、爆炸特效/声音、SDKDamage 实际数值，以及直接速度击退不会产生 Charger 式硬直。用临时输出做同源码编译时，编译退出码为 `0`，但 `.smx` 的 SHA-256 每次可能不同，因此未把二进制哈希相等作为验收条件。
