	...
	switch(iTeamNoticeType)
	{
		case 0:
		{
			for(new i = 1; i <= MaxClients;i++)
			{
				...
				if(iNoticeType == 1)
				PrintToChat(i, "\x04[B&W] \x03%N\x04(\x03%s\x04) 已经黑白了", iClient, sCharName);
				if(iNoticeType == 2)
				PrintHintText(i, "[B&W] %N(%s\x04) 已经黑白了", iClient, sCharName);
				if(iNoticeType == 3)
		...
		case 2:
		{
			for(new i = 1; i <= MaxClients;i++)
			{
				...
				PrintToChat(i, "\x04[B&W] \x03%N\x04(\x03%s\x04) 已经黑白了", iClient, sCharName);
				if(iNoticeType == 2)
				PrintHintText(i, "[B&W] %N(%s) 已经黑白了", iClient, sCharName);
	...
	switch(iTeamNoticeType)
	{
		case 0:
		{
			for(new i = 1; i <= MaxClients;i++)
			{
				...
				if(iNoticeType == 1)
				if(iClient != iHealer)
				PrintToChat(i, "\x04[B&W] \x03%N\x04(\x03%s\x04) 不再黑白", iClient, sCharName);
				else
				PrintToChat(i, "\x04[B&W] \x03%N\x04(\x03%s\x04) 治疗了自己", iClient, sCharName);
				
				if(iNoticeType == 2)
				if(iClient != iHealer)
				PrintHintText(i, "[B&W] %N(%s) 不再黑白", iClient, sCharName);
				else
				PrintHintText(i, "[B&W] %N(%s) 治疗了自己", iClient, sCharName);
		...
		case 1:
		{
			for(new i = 1; i <= MaxClients;i++)
			{
				...
				if(iNoticeType == 2)
				if(iClient != iHealer)
				PrintHintText(i, "[B&W] %N(%s) 不再黑白", iClient);
				else
...
static DirectorHint(iClient, i)
{
	...
	FormatEx(sValues, sizeof(sValues), "%N(%s) 已经黑白了", iClient, sCharName);
...
static DirectorHintAll(iClient, iHealer, i)
{
	...
	if(iClient == iHealer)
	FormatEx(sValues, sizeof(sValues), "%N(%s) 治疗了自己", iClient, sCharName);
	else
	FormatEx(sValues, sizeof(sValues), "%N(%s) 不再黑白", iClient, sCharName);
