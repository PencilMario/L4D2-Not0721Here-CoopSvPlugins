...
public void Event_WitchSpawn(Event event, const char[] name, bool dontBroadcast)
{
	...
	//{
	//	case 1:
	//		SetWitchHealth(Witchid, g_iWitchHealth);
	//}
}

//void SetWitchHealth(int Witchid, int iHealth)
//{
//	SetClientHealth(Witchid, iHealth);
//	if (!TankSpawnFinaleVehicleLeaving)
//		CPrintToChatAll("{default}[{blue}!{default}] {blue}%s{default}出现,{blue}血量为:{green}%d", GetWitchName(Witchid), iHealth);//聊天窗提示.
//}
