...
**Architecture:** `state.inc` owns coordinator state, global snapshot expiry, and per-Bot cooldown. `runtime.inc` refreshes shared client snapshots only through `GetProcessCacheExpiry()`. `rescue.inc` receives grab/release lifecycle events, builds nearest-Bot ranks once per event from those snapshots, exposes O(1) assignment checks, and supplies specialized Jockey/Smoker aim positions. `events.inc` translates game events; `bot_think.inc` keeps the existing action decisions behind an early gate and uses specialized aim only for Jockey/Smoker.
...
**Compatibility Boundary:** Preserve nearest-rank and tie-break behavior, cvar zero-disable semantics, reaction delay, shoot/shove range and weapon guards, visibility blindness rules, native rescue actions, generic targeting, and include order. Shared global snapshots use the `ib_process_time` expiry as their only refresh clock. Retire only the per-usercmd pinned-attacker discovery and generic Jockey/Smoker bone fallback.
...
- [x] **Step 1: Add the failing coordinator contract.**
...
- [x] **Step 3: Add coordinator state and reset ownership.**

  Add the arrays to `state.inc`; extend `ResetPinnedReactionCaches` and `ResetClientPluginVariables`/map lifecycle calls so active rows, assignments, ranks, global snapshot expiry, and cooldowns cannot survive a client or map boundary.

- [x] **Step 4: Centralize shared global snapshot refresh.**

  Add `UpdateClientGlobalInfo` in `runtime.inc`. It must return while `GetGameTime() < g_fClientGlobalInfo_Expiry[iClient]`, refresh all four shared client snapshots once, and assign `g_fClientGlobalInfo_Expiry[iClient] = GetProcessCacheExpiry()`. Event code and rescue ranking consume these snapshots without direct position refresh calls.

- [x] **Step 5: Implement event-driven transitions.**
...
- [x] **Step 6: Connect grab, release, death, revive, and client lifecycle events.**

  Pass `userid` attacker plus `victim` to the start transition; hook `pounce_end`, `tongue_release`, `jockey_ride_end`, `charger_carry_end`, and `charger_pummel_end` to invalidation. Invalidate on player death and revive, and also invalidate on `player_spawn`, `player_team`, `player_bot_replace`, and `bot_player_replace`. Those lifecycle events request a coordinator recovery; the runtime performs it only after all alive team-2/team-3 snapshots are ready. Clear the assignment owned by a client slot during client invalidation so slot reuse cannot inherit a stale rescue target.

- [x] **Step 7: Run focused and existing rescue contracts.**
...
- Modify: `addons/sourcemod/scripting/l4d2_sb_ai_improver/state.inc`.
- Modify: `addons/sourcemod/scripting/l4d2_sb_ai_improver/runtime.inc`.
...
**Repair Track:** Add `ib_help_pinned_reaction_interval` defaulting to `0.15` seconds, use the existing snapshots refreshed by `ib_process_time`, use the cached centroids for Jockeys, and use valid Smoker `m_tipPosition`/centroid data. Keep `HasVisualContactWithEntity` and generic `GetTargetAimPart` for other rescue classes.
...
- [x] **Step 1: Extend the failing contract with global snapshot, cooldown, and aim assertions.**

  Assert the shared snapshot expiry/helper and `GetProcessCacheExpiry` assignment, the cvar declaration/cache assignment, the per-Bot next-time array and reset, `GetPinnedRescueAimPosition`, `m_tipPosition`, `IsVisibleVector`, and the rescue branch's `g_fSurvivorBot_NextPinnedRescueThinkTime` guard.

- [x] **Step 2: Run the focused contract and confirm the expected RED failure.**

  The test must fail on the first absent snapshot/cooldown/aim token rather than a PowerShell error.

- [x] **Step 3: Add and cache the interval cvar.**
...
- [x] **Step 4: Add the specialized aim helper.**
...
- [x] **Step 5: Add the early gate and cooldown.**
...
- [x] **Step 6: Route Jockey/Smoker through the helper.**
...
- [x] **Step 7: Run focused contracts and compile.**
...
- [x] **Step 3: Review side effects and evidence.**
