...
| --- | --- | --- | --- | --- | --- | --- |
| 战役 | `coop_base` | 基础战役 | 3 / 35 秒 | +1 / -2 秒 | 允许 | `3` 修正 / 350 |
| 战役 | `coop_hard` | 逛街多人多特 | 8 / 15 秒 | +2 / 不变 | 允许 | `3` 修正 / 350 |
| 战役 | `coop_fire` | 无限火力 | 8 / 0 秒 | +2 / 不变 | 跳过 | `3` 修正 / 350 |
| 战役 | `coop_himiko` | 秘密子能带飞 | 14 / 15 秒 | +2 / 不变 | 允许 | `3` 修正 / 350 |
| 战役 | `community5_multi` | 多人逛街死门 | 5 / 0 秒 | +1 / 不变 | 跳过 | `3` 修正 / 350 |
| 写实 | `realism_solo` | 写专单通 | 14 / 15 秒 | 固定 | 允许 | `3` 修正 / 350 |
| 绝境 | `mutation4_noobplus` | 逛街绝境混野版 | 8 / 15 秒 | 固定 | 允许 | `3` 修正 / 350 |
| 绝境 | `mutation4_solo` | 绝境单通 | 8 / 15 秒 | 固定 | 允许 | `3` 修正 / 350 |
| 绝境 | `mutation4_ez` | 逛街14特 | 14 / 15 秒 | 固定 | 允许 | `3` 修正 / 350 |
...
| 死门 | `community5_ez` | 逛街死门12特0秒 | 12 / 0 秒 | 固定 | 跳过 | `3` 修正 / 350 |
...
| 开牢！ | `coop_annelike` | 战役\|饼干的Anne改战役 | 6 / 16 秒 | +1 / -2 秒 | 允许 | `3` 修正 / 350 |
...
本插件包将 `z_pounce_damage_interrupt` 设为 150。总览中的“`3` 修正 / 350”表示 AI Hunter 同一次飞扑累计受伤达到 150 时触发空爆、移除 AI Charger 冲锋减伤，并将 survivor 对 Charger 的近战伤害固定为每挥 350。最后一击会按 Charger 剩余血量封顶；“`0` 原版 / 不固定”表示不添加 `l4d2_melee_damage_charger`，保留原版命中部位伤害。
