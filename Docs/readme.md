...
| 系列 | 配置 | 显示名称 | 基础刷特 | 人数增长 | Relax | AI 伤害修正 |
| --- | --- | --- | --- | --- | --- | --- |
| 战役 | `coop_base` | 基础战役 | 3 / 35 秒 | +1 / -2 秒 | 允许 | `3` 修正 |
| 战役 | `coop_hard` | 逛街多人多特 | 8 / 15 秒 | +2 / 不变 | 允许 | `3` 修正 |
| 战役 | `coop_fire` | 无限火力 | 8 / 0 秒 | +2 / 不变 | 跳过 | `3` 修正 |
| 战役 | `coop_himiko` | 秘密子能带飞 | 14 / 15 秒 | +2 / 不变 | 允许 | `3` 修正 |
| 战役 | `community5_multi` | 多人逛街死门 | 5 / 0 秒 | +1 / 不变 | 跳过 | `3` 修正 |
| 写实 | `realism_solo` | 写专单通 | 14 / 15 秒 | 固定 | 允许 | `3` 修正 |
| 绝境 | `mutation4_noobplus` | 逛街绝境混野版 | 8 / 15 秒 | 固定 | 允许 | `3` 修正 |
| 绝境 | `mutation4_solo` | 绝境单通 | 8 / 15 秒 | 固定 | 允许 | `3` 修正 |
| 绝境 | `mutation4_ez` | 逛街14特 | 14 / 15 秒 | 固定 | 允许 | `3` 修正 |
| 绝境 | `mutation4` | 绝境14特 | 14 / 15 秒 | 固定 | 允许 | `0` 原版 |
| 绝境 | `mutation4_except` | 绝境28特6控 | 28 / 15 秒 | 固定 | 允许 | `0` 原版 |
| 死门 | `community5_noobplus` | 逛街死门混野版 | 5 / 0 秒 | 固定 | 跳过 | `3` 修正 |
| 死门 | `community5_ez` | 逛街死门12特0秒 | 12 / 0 秒 | 固定 | 跳过 | `3` 修正 |
| 死门 | `community5` | 死门10特0秒 | 10 / 0 秒 | 固定 | 跳过 | `0` 原版 |
| 死门 | `community5_himiko` | 秘密子来了都过了 | 24 / 0 秒 | 固定 | 跳过 | `3` 修正 |
| 开牢！ | `community5_610` | 死门\|6特10s | 6 / 10 秒 | 固定 | 跳过 | `0` 原版 |
| 开牢！ | `community5_jimen` | 几门\|群都死了 | 6 / 15 秒 | 固定 | 跳过 | `0` 原版 |
| 开牢！ | `realism_miaomei` | 写专\|秒妹老师の4k血妹 | 3 / 45 秒 | 固定 | 允许 | `0` 原版 |
| 开牢！ | `coop_fuckmap` | 战役\|什么吊图 | 6 / 15 秒 | +2 / -2 秒 | 允许 | `0` 原版 |
| 开牢！ | `coop_annelike` | 战役\|饼干的Anne改战役 | 6 / 16 秒 | +1 / -2 秒 | 允许 | `3` 修正 |
| 开牢！ | `community2` | 感染季节\|绝境14特 | 14 / 15 秒 | 固定 | 允许 | `0` 原版 |
| 开牢！ | `coop_wtf` | 战役\|你们玩这么变态的？ | 10 / 0 秒 | +1 / 不变 | 跳过 | `0` 原版 |
| 开牢！ | `coop_rpg` | 战役\|Sky rpg v3.4.7.8 | 5 / 15 秒 | +1 / -2 秒 | 允许 | `0` 原版 |
| 开牢！ | `versus_isfullshit` | 对抗\|救世主大战逆天特感争抢0721 | 8 / 15 秒 | 固定 | 允许 | `0` 原版 |
...
本插件包将 `z_pounce_damage_interrupt` 设为 150。因此总览中的“`3` 修正”表示 AI Hunter 同一次飞扑累计受伤达到 150 时触发空爆，同时移除 AI Charger 冲锋减伤；“`0` 原版”表示保留这两项原版 AI 行为。
