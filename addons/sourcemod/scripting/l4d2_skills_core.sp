...
public Action sm_skills_give_team_money( int client, int args )
{
	...
	Skills_PrintToChatAll("\x01增加 \x04%.0f \x01至队伍积分", add);
	return Plugin_Handled;
}
...
public Action sm_skills_give_money( int client, int args )
{
	...
	Skills_PrintToChatAll("\x05%N \x04给了 \x05%N \x04积分 \x03%.0f", client, target, add);
	return Plugin_Handled;
}
...
public Action sm_skills_give_skill( int client, int args )
{
	...
	Skills_PrintToChatAll("\x05%N \x04给予 \x05%N \x04技能 \x03%%s", client, target, name);
	return Plugin_Handled;
}
...
public Action sm_skills_list( int client, int args )
{
	...
	if ( !g_Skills.count )
	{
		Skills_ReplyToCommand(client, "无技能注册");
		return Plugin_Handled;
	}
